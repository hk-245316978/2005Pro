System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Node, PolygonCollider2D, Intersection2D, UITransform, v3, v2, _dec, _class2, _crd, ccclass, property, eventTarget, _class;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "0779agi9ExFY64wGlytHVMs", "MoveSprite", undefined);
      ...
      _export("default", _class = (_dec = ccclass('MoveSprite'), _dec(_class2 = class _class2 extends Component {
        ...
        event_mouse_down(event) {
          ...
          var collider = this.node.getComponent(PolygonCollider2D); // 如果没有碰撞组件直接返回
          ...
          var downPoint = event.getUILocation(); // 把UI窗口内的坐标位置转换成节点为中心点的坐标

          var nodePoint = this.node.getComponent(UITransform).convertToNodeSpaceAR(v3(downPoint.x, downPoint.y)); // 多边形的顶点

          var points = collider.points; // 获取点击位置的坐标是否在多边形内

          var is = Intersection2D.pointInPolygon(v2(nodePoint.x, nodePoint.y), points);
        ...
        event_mouse_move(event) {
          if (this.isMoving && this.isActive) {
            var movePointN = v2(event.getLocation().x - this.mousePoint.x, event.getLocation().y - this.mousePoint.y);
...
//# sourceMappingURL=bf98893e0555c5b40d47843061714e13227523bb.js.map